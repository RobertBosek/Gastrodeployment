...
class RealsenseD435Camera:
    ...
    def init_video_capture(self):
        try:
            ...
            depth_ir_sensor.set_option(rs.option.exposure, 400)
            depth_ir_sensor.set_option(rs.option.gain, 16)
            ...
            if self.node is not None and DEBUG_MODE:
    ...
    def update(self):
        ...
        while self.started:
            ...
            if not color_frame or not left_ir_image:  # or not aligned_depth_frame:
                continue
            ...
            if self.camera_matrix_rgb is not None and self.dist_matrix_rgb is not None:
            ...
            with self.read_lock:
                ...
                if self.node is not None:
                    self.node.publisher_rgb.publish(self.node.cv_bridge.cv2_to_imgmsg(self.color_image, "passthrough"))
                    self.node.publisher_ir.publish(self.node.cv_bridge.cv2_to_imgmsg(self.left_ir_image, "passthrough"))
